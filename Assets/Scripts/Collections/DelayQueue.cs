...
namespace NuRpg.Collections {
	public class DelayQueue<T> {
		...
		private readonly PriorityQueue<T, DateTimeSpan> _queue;
		...
		*/
		...
		public bool Ready => _queue.TryPeek(out _, out var span) && span.End <= _time;

		public DateTime Time => _time;

		public PriorityQueue<T, DateTimeSpan>.UnorderedItemsCollection UnorderedItems => _queue.UnorderedItems;
		...
		public void Enqueue(T element, TimeSpan duration) {
			Enqueue(element, new DateTimeSpan(_time, _time + duration));
		}

		public void EnqueueRange(IEnumerable<(T Element, DateTimeSpan Span)> items) {
			_queue.EnqueueRange(items);
		}

		public void EnqueueRange(IEnumerable<T> elements, DateTimeSpan span) {
			_queue.EnqueueRange(elements, span);
		}

		public void EnqueueRange(IEnumerable<T> elements, TimeSpan duration) {
			EnqueueRange(elements, new DateTimeSpan(_time, _time + duration));
		}
		...
		public IEnumerator<(T Element, DateTimeSpan Span)> GetEnumerator() {
			return UnorderedItems.GetEnumerator();
		}
		...
		public bool TryDequeue([MaybeNullWhen(false)] out T element, out DateTimeSpan span) {
			if( _queue.TryDequeue(out element, out span) ) {
				return true;
			}
			else {
				element = default;
		...
		public bool TryPeek([MaybeNullWhen(false)] out T result, out DateTimeSpan span) {
			if( _queue.TryPeek(out result, out span) && span.End <= _time ) {
				return true;
			}
			else {
				result = default;
